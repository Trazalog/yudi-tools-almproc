...
class Almproc_tareas extends CI_Model
{
    ...
    public function desplegarVista($tarea)
    {
        switch ($tarea->nombreTarea) {
                ...
                case 'Limpieza y revisión inicial':
                    ...
                    return $this->load->view(ALM . 'proceso/tareas/pedido_reparacion_neumaticos/view_aprueba_reparacion', $data, true);
                ...
                case 'Escariado':
                    ...
                    return $this->load->view(ALM . 'proceso/tareas/pedido_reparacion_neumaticos/view_escariado', $data, true);
...
#YUDI #CHUKA
    // public function pedidoReparacion($caseId)
    // {
    //     //? DEBE EXISTIR LA NOTA DE PEDIDO
    //     $contract = [
    //         'apruebaTrabajo' => true,
    //         'esEmparchadoMenor'=> true,
    //     ];

    //     $rsp['data'] = $this->bpm->lanzarProceso(BPM_PROCESS_ID_REPARACION_NEUMATICOS, $contract);
...
/////////////////#YUDI #CHUKA
public function aceptarProcesoYUDI($case_id)
    {
        $taskId = $this->bpm->ObtenerTaskidXNombre(BPM_PROCESS_ID_REPARACION_NEUMATICOS, $case_id, 'Limpieza y revisión inicial');
        log_message('DEBUG', 'YUDI CHUKA --(ObtenerTaskidXNombre) #$taskId->' . $taskId);
        $user = userId();
        if ($taskId && $user) {
...
//     if ($taskId && $user) {
//         $resultSetUsuario = $this->bpm->setUsuario($taskId, $user);
//         $contract["apruebaTrabajo"] = true;
//         $contract["esEmparchadoMenor"] = true;
...
#YUDI #CHUKA
            case 'Limpieza y revisión inicial':
                log_message('DEBUG', 'YUDI CHUKA -Limpieza y revisión inicial ');
...
#YUDI #CHUKA
            case 'Escariado':
                log_message('DEBUG', 'YUDI CHUKA -Escariado');
                $taskId = $this->bpm->ObtenerTaskidXNombre(BPM_PROCESS_ID_REPARACION_NEUMATICOS,  $taskId, 'Limpieza y revisión inicial');
                log_message('DEBUG', 'YUDI CHUKA --(ObtenerTaskidXNombre) #$taskId->' . $taskId);
            //    $user = userId();
                  //      $resultSetUsuario = $this->bpm->setUsuario($taskId, $user);
